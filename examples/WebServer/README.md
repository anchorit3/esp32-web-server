...
This example shows different techniques on how to use and extend the ESP32WebServer for specific purposes
...
It requires some space for a filesystem and runs fine on ESP32 NodeMCU board with 4 MByte flash using the following options:
...
The ESP32WebServer library offers a simple path to implement a web server on a ESP32 board.

The advantage on using the ESP32WebServer instead of the plain simple WiFiServer is that the ESP32WebServer
