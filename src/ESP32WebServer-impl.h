...
ESP32WebServerTemplate<ServerType>::ESP32WebServerTemplate(IPAddress addr, int port)
: _server(addr, port)
{
}

template <typename ServerType>
ESP32WebServerTemplate<ServerType>::ESP32WebServerTemplate(int port)
: _server(port)
{
}

template <typename ServerType>
ESP32WebServerTemplate<ServerType>::~ESP32WebServerTemplate() {
  _server.close();
  if (_currentHeaders)
...
template <typename ServerType>
void ESP32WebServerTemplate<ServerType>::enableCORS(bool enable) {
  _corsEnabled = enable;
}

template <typename ServerType>
void ESP32WebServerTemplate<ServerType>::enableETag(bool enable, ETagFunction fn) {
  _eTagEnabled = enable;
  _eTagFunction = fn;
}

template <typename ServerType>
void ESP32WebServerTemplate<ServerType>::begin() {
  close();
  _server.begin();
}

template <typename ServerType>
void ESP32WebServerTemplate<ServerType>::begin(uint16_t port) {
  close();
  _server.begin(port);
}

template <typename ServerType>
String ESP32WebServerTemplate<ServerType>::_extractParam(String& authReq,const String& param,const char delimit) const {
...
template <typename ServerType>
bool ESP32WebServerTemplate<ServerType>::authenticate(const char * username, const char * password){
...
template <typename ServerType>
bool ESP32WebServerTemplate<ServerType>::authenticateDigest(const String& username, const String& H1)
{
  if(hasHeader(FPSTR(AUTHORIZATION_HEADER))) {
    String authReq = header(FPSTR(AUTHORIZATION_HEADER));
...
template <typename ServerType>
String ESP32WebServerTemplate<ServerType>::_getRandomHexString() {
...
template <typename ServerType>
void ESP32WebServerTemplate<ServerType>::requestAuthentication(HTTPAuthMethod mode, const char* realm, const String& authFailMsg) {
  if(realm == NULL) {
...
template <typename ServerType>
void ESP32WebServerTemplate<ServerType>::on(const Uri &uri, ESP32WebServerTemplate<ServerType>::THandlerFunction handler) {
  on(uri, HTTP_ANY, handler);
}

template <typename ServerType>
void ESP32WebServerTemplate<ServerType>::on(const Uri &uri, HTTPMethod method, ESP32WebServerTemplate<ServerType>::THandlerFunction fn) {
  on(uri, method, fn, _fileUploadHandler);
}

template <typename ServerType>
void ESP32WebServerTemplate<ServerType>::on(const Uri &uri, HTTPMethod method, ESP32WebServerTemplate<ServerType>::THandlerFunction fn, ESP32WebServerTemplate<ServerType>::THandlerFunction ufn) {
  _addRequestHandler(new FunctionRequestHandler<ServerType>(fn, ufn, uri, method));
}

template <typename ServerType>
void ESP32WebServerTemplate<ServerType>::addHandler(RequestHandlerType* handler) {
    _addRequestHandler(handler);
}

template <typename ServerType>
void ESP32WebServerTemplate<ServerType>::_addRequestHandler(RequestHandlerType* handler) {
    if (!_lastHandler) {
...
template <typename ServerType>
void ESP32WebServerTemplate<ServerType>::serveStatic(const char* uri, FS& fs, const char* path, const char* cache_header) {
  bool is_file = false;
...
template <typename ServerType>
void ESP32WebServerTemplate<ServerType>::handleClient() {
...
template <typename ServerType>
void ESP32WebServerTemplate<ServerType>::close() {
  _server.close();
  _currentStatus = HC_NONE;
  if(!_headerKeysCount)
    collectHeaders();
...
template <typename ServerType>
void ESP32WebServerTemplate<ServerType>::stop() {
  close();
}

template <typename ServerType>
void ESP32WebServerTemplate<ServerType>::sendHeader(const String& name, const String& value, bool first) {
...
template <typename ServerType>
void ESP32WebServerTemplate<ServerType>::setContentLength(const size_t contentLength) {
    _contentLength = contentLength;
}

template <typename ServerType>
void ESP32WebServerTemplate<ServerType>::_prepareHeader(String& response, int code, const char* content_type, size_t contentLength) {
...
template <typename ServerType>
void ESP32WebServerTemplate<ServerType>::send(int code, char* content_type, const String& content) {
  return send(code, (const char*)content_type, content);
}

template <typename ServerType>
void ESP32WebServerTemplate<ServerType>::send(int code, const char* content_type, const String& content) {
  return send(code, content_type, content.c_str(), content.length());
}

template <typename ServerType>
void ESP32WebServerTemplate<ServerType>::send(int code, const String& content_type, const String& content) {
  return send(code, (const char*)content_type.c_str(), content);
}

template <typename ServerType>
void ESP32WebServerTemplate<ServerType>::sendContent(const String& content) {
  StreamConstPtr ref(content.c_str(), content.length());
  sendContent(&ref);
}

template <typename ServerType>
void ESP32WebServerTemplate<ServerType>::send(int code, const char* content_type, Stream* stream, size_t content_length /*= 0*/) {
  String header;
  if (content_length == 0)
...
template <typename ServerType>
void ESP32WebServerTemplate<ServerType>::send_P(int code, PGM_P content_type, PGM_P content) {
  StreamConstPtr ref(content, strlen_P(content));
  return send(code, String(content_type).c_str(), &ref);
}

template <typename ServerType>
void ESP32WebServerTemplate<ServerType>::send_P(int code, PGM_P content_type, PGM_P content, size_t contentLength) {
  StreamConstPtr ref(content, contentLength);
  return send(code, String(content_type).c_str(), &ref);
}

template <typename ServerType>
void ESP32WebServerTemplate<ServerType>::sendContent(Stream* content, ssize_t content_length /* = 0*/) {
...
template <typename ServerType>
void ESP32WebServerTemplate<ServerType>::sendContent_P(PGM_P content) {
  sendContent_P(content, strlen_P(content));
}

template <typename ServerType>
void ESP32WebServerTemplate<ServerType>::sendContent_P(PGM_P content, size_t size) {
  StreamConstPtr ptr(content, size);
  return sendContent(&ptr, size);
}

template <typename ServerType>
String ESP32WebServerTemplate<ServerType>::credentialHash(const String& username, const String& realm, const String& password)
{
  MD5Builder md5;
  md5.begin();
...
template <typename ServerType>
void ESP32WebServerTemplate<ServerType>::_streamFileCore(const size_t fileSize, const String &fileName, const String &contentType)
{
  using namespace mime;
...
template <typename ServerType>
const String& ESP32WebServerTemplate<ServerType>::pathArg(unsigned int i) const {
...
template <typename ServerType>
const String& ESP32WebServerTemplate<ServerType>::arg(const String& name) const {
...
template <typename ServerType>
const String& ESP32WebServerTemplate<ServerType>::arg(int i) const {
...
template <typename ServerType>
const String& ESP32WebServerTemplate<ServerType>::argName(int i) const {
  if (i >= 0 && i < _currentArgCount + _currentArgsHavePlain)
...
template <typename ServerType>
int ESP32WebServerTemplate<ServerType>::args() const {
  return _currentArgCount;
}

template <typename ServerType>
bool ESP32WebServerTemplate<ServerType>::hasArg(const String& name) const {
  for (int j = 0; j < _postArgsLen; ++j) {
    if (_postArgs[j].key == name)
      return true;
  }
...
template <typename ServerType>
const String& ESP32WebServerTemplate<ServerType>::header(const String& name) const {
...
template<typename ServerType>
void ESP32WebServerTemplate<ServerType>::collectHeaders(const char* headerKeys[], const size_t headerKeysCount) {
  if (_currentHeaders)
    delete[] _currentHeaders;
...
void ESP32WebServerTemplate<ServerType>::collectHeaders(const Args&... args) {
  if (_currentHeaders)
    delete[] _currentHeaders;
...
template <typename ServerType>
const String& ESP32WebServerTemplate<ServerType>::header(int i) const {
...
template <typename ServerType>
const String& ESP32WebServerTemplate<ServerType>::headerName(int i) const {
  if (i < _headerKeysCount)
...
template <typename ServerType>
int ESP32WebServerTemplate<ServerType>::headers() const {
  return _headerKeysCount;
}

template <typename ServerType>
bool ESP32WebServerTemplate<ServerType>::hasHeader(const String& name) const {
  for (int i = 0; i < _headerKeysCount; ++i) {
...
template <typename ServerType>
const String& ESP32WebServerTemplate<ServerType>::hostHeader() const {
  return _hostHeader;
}

template <typename ServerType>
void ESP32WebServerTemplate<ServerType>::onFileUpload(THandlerFunction fn) {
  _fileUploadHandler = fn;
}

template <typename ServerType>
void ESP32WebServerTemplate<ServerType>::onNotFound(THandlerFunction fn) {
  _notFoundHandler = fn;
}

template <typename ServerType>
void ESP32WebServerTemplate<ServerType>::_handleRequest() {
  bool handled = false;
  if (!_currentHandler){
...
template <typename ServerType>
void ESP32WebServerTemplate<ServerType>::_finalizeResponse() {
  if (_chunked) {
...
template <typename ServerType>
String ESP32WebServerTemplate<ServerType>::responseCodeToString(const int code) {
